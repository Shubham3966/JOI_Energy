...
The project makes use of Maven and uses the [Maven wrapper](https://docs.Maven.org/current/userguide/Maven_wrapper.html), which means you don't need Maven
installed.
...
List all the tasks that Maven can run, such as `build` and `test`.
...
Run the application using Java and the executable JAR file produced by the Maven `build` task. The application will be
